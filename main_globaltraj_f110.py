...
parser.add_argument(
    "--map_name", type=str, default="speedcar_sim_track_b", help="Name of the map (default: speedcar_sim_track_b)"
)
parser.add_argument(
    "--map_path",
    type=str,
    default="",
    help="Path to the map centerline (should be a .csv), defaults to inputs/tracks/<map_name>.csv",
)
...
file_paths = {"veh_params_file": "speedcar_sim.ini"}
...
file_paths["tpamap"] = os.path.join(
    file_paths["module"], "inputs", "frictionmaps", file_paths["track_name"] + "_tpamap.csv"
)

if mintime_opts["tpadata"] is None:
    file_paths["tpadata"] = os.path.join(
        file_paths["module"], "inputs", "frictionmaps", file_paths["track_name"] + "_tpadata.json"
    )
else:
...
file_paths["lap_time_mat_export"] = os.path.join(
    file_paths["module"], f"outputs/{MAP_NAME}", lap_time_mat_opts["file"]
)
...
if opt_type == "mintime" and mintime_opts["reopt_mintime_solution"]:
    ...
    if debug:
        ...
        bound_r_reopt = (
            reftrack_interp[:, :2] + np.expand_dims(reftrack_interp[:, 2], axis=1) * normvec_normalized_interp
        )
        bound_l_reopt = (
            reftrack_interp[:, :2] - np.expand_dims(reftrack_interp[:, 3], axis=1) * normvec_normalized_interp
        )
...
if lap_time_mat_opts["use_lap_time_mat"]:
    ...
    ggv_scales = np.linspace(
        ...
        int(
            (lap_time_mat_opts["gg_scale_range"][1] - lap_time_mat_opts["gg_scale_range"][0])
    ...
    top_speeds = np.linspace(
        ...
        int(
            (lap_time_mat_opts["top_speed_range"][1] - lap_time_mat_opts["top_speed_range"][0])
    ...
    for i, top_speed in enumerate(top_speeds):
        for j, ggv_scale in enumerate(ggv_scales):
            tph.progressbar.progressbar(
                i * ggv_scales.shape[0] + j, top_speeds.shape[0] * ggv_scales.shape[0], prefix="Simulating laptimes "
            )
...
if "traj_race_export" in file_paths.keys():
    helper_funcs_glob.src.export_traj_race.export_traj_race_f110(file_paths=file_paths, traj_race=traj_race_cl)
